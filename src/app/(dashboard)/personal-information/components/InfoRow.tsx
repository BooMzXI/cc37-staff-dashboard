...
export const InfoRow = ({ label, value }: { label: string; value: string | number | boolean | null | undefined }) => {
	const displayValue = () => {
		if (value === null || value === undefined || value === "") return "-";

		if (typeof value === "boolean") {
			return value ? <CircleCheckBig className="w-4 h-4 text-green-500" /> : <CircleX className="w-4 h-4 text-red-500" />;
		}
